...
# app/services/rewards_service.py
# app/services/rewards/rewards_service.py
from typing import Dict, Any
from decimal import Decimal
from sqlalchemy.orm import Session
from app.db import crud_rewards
...
def decompose_to_badges(total_events: int):
    """
    - 4 events = 1 Gold (stackable)
    - Remaining 3 = 1 Silver
    - Remaining >= 1 = 1 Bronze
    """
    gold = total_events // REWARDS["gold"]["threshold"]
    rem = total_events - gold * REWARDS["gold"]["threshold"]
...
def generate_coupons_for_user(db: Session, user_id: int, counts: Dict[str, int]):
    """
    Issues coupons based on badge breakdown.
    Prevents duplicate coupon spam.
    """
    total = counts["total_events"]
    badges = decompose_to_badges(total)
    issued = []

    # Existing unredeemed coupons
    existing = crud_rewards.get_user_unredeemed_coupons(db, user_id)
    existing_counts = {}
    for c in existing:
        level = c.status_level.lower()
        existing_counts[level] = existing_counts.get(level, 0) + 1

    # GOLD (stackable)
    for _ in range(badges["gold"]):
        already = existing_counts.get("gold", 0)
        if already > 0:
            existing_counts["gold"] -= 1
            continue
        cp = crud_rewards.issue_coupon(
            db, user_id, "Gold", 8.00, True,
            meta={"reason": "earned_4_events_30d"}
        )
        issued.append(cp)
    ...
    for _ in range(badges["silver"]):
        already = existing_counts.get("silver", 0)
        if already > 0:
            existing_counts["silver"] -= 1
            continue
        cp = crud_rewards.issue_coupon(
            db, user_id, "Silver", 4.00, False,
            meta={"reason": "earned_3_events_30d"}
        )
        issued.append(cp)
    ...
    if badges["bronze"]:
        ...
        if already == 0:
            cp = crud_rewards.issue_coupon(
                db, user_id, "Bronze", 0.00, False,
                meta={"reason": "earned_1_event_30d"}
            )
            issued.append(cp)
...
def compute_user_rewards_and_progress(db: Session, user_id: int) -> Dict[str, Any]:
    """
    Returns full JSON for GET /rewards/suggestion
    """
    counts = crud_rewards.get_user_activity_counts(db, user_id)
    badges = decompose_to_badges(counts["total_events"])

    # Progress toward next Gold
    progress_to_next = REWARDS["gold"]["threshold"] - (counts["total_events"] % 4)
    ...
    if badges["gold"]:
        current_status.append({
            ...
            "count": badges["gold"],
            "reward": "8% discount",
            "criteria": "4 events in last 30 days",
            "stackable": True
        })
    ...
    if badges["silver"]:
        current_status.append({
            ...
            "count": badges["silver"],
            "reward": "4% discount",
            "criteria": "3 events in last 30 days"
        })
    ...
    if badges["bronze"]:
        current_status.append({
            ...
            "count": 1,
            "reward": "Badge only",
            "criteria": "1 event in last 30 days"
        })
    ...
    return {
        ...
        "progress": {
            ...
            "total_events": counts["total_events"],
            "next_status": "Gold",
            "events_needed": int(progress_to_next),
        },
        "available_discounts": coupons,
        "history": crud_rewards.get_status_history_list(db, user_id),
    }
